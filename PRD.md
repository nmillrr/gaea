...
6. ✅ **Reconcile the scoring curve** with PRD §10 — *done.* `src/game/scoring.ts` rewritten from a linear/20 km cutoff to the §10 nonlinear band curve (5,000 perfect → 0–149 beyond 100 km) via piecewise-linear interpolation; signature unchanged, so the guess controller picks it up automatically. Test suite rewritten to assert the bands + curve properties (12 tests, 100% coverage on `scoring.ts`).
7. 🔶 **Game-depth backbone** — *UI screens done; game systems pending.*
   - ✅ **Splash / init screen** — `App.tsx` `LoadingScreen` now shows the centered `gaea` wordmark (init_screen); the feed skeleton already covers loading_feed.
   - ✅ **Notifications / Activity screen** — new `NotificationsScreen` (Today / Earlier sections, actor + "and N others" + post thumbnail) backed by a real `GET /notifications/activity` endpoint that aggregates guesses (per-post) and comments on the user's own posts. Wired to the feed's bell icon. *(Also resolved the long-standing `App.tsx` union-Props error by consolidating the dead `Home` route into `Feed`.)*
   - ⏳ **Still pending (heavier infra — own milestone):** 24-hour **reveal/timer** + the scheduled job to fire it, **streaks** (+ shields), and **weekly/all-time per-group leaderboards**. These need new entities and a job runner, so they warrant a dedicated planning pass rather than being folded into this UI sweep.
   - ✅ **Groups** (entity + membership + invites) — *done.* New `Group` and `GroupMember` entities with composite PK, `groupController.ts` (create, list, get, addMember, removeMember), `/groups` routes, and 20-test unit suite. Enforces PRD §8 (20-member cap) and §14 (3-group free-tier limit). Owner-only invites, self-leave, membership gating on detail view.
