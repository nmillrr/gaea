...
7. 🔶 **Game-depth backbone** — *UI screens done; game systems pending.*
   ...
   - ⏳ **Still pending (heavier infra — own milestone):** 24-hour **reveal/timer** + the scheduled job to fire it, **streaks** (+ shields), **groups** (entity + membership + invites), and **weekly/all-time per-group leaderboards**. These need new entities and a job runner, so they warrant a dedicated planning pass rather than being folded into this UI sweep.
